...
/// delivers nothing, so a deployment running it has a password reset flow that issues valid tokens
/// no Employee ever receives. That is the honest state of the system at this milestone, and a
/// class called <c>SmtpPasswordResetNotifier</c> that quietly dropped the message would be worse
...
/// </para>
/// </remarks>
internal sealed partial class UndeliveredPasswordResetNotifier(
    ILogger<UndeliveredPasswordResetNotifier> logger) : IPasswordResetNotifier
{
    /// <inheritdoc />
        ...
        CancellationToken cancellationToken)
    {
        ...
        PasswordResetNotDelivered(logger, employeeId.ToString(), expiresAtUtc);
    ...
    private static partial void PasswordResetNotDelivered(
